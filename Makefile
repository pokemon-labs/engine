...
.PHONY: zig-build
zig-build:
	zig build -fsummary -Dlog -Dchance -p build
	zig build -fsummary -Dshowdown -Dlog -Dchance -p build
...
.PHONY: zig-test
zig-test:
	zig build -fsummary -Dlog -Dchance test
	zig build -fsummary -Dshowdown -Dlog -Dchance test
...
zig-coverage:
	...
	zig build -fsummary -Dshowdown -Dlog -Dchance test -Dtest-coverage=coverage/zig/pkmn-showdown
...
.PHONY: t
t:
	zig build -fsummary test -Dlog -Dchance -Dtest-file=src/lib/gen$(gen)/test.zig -Dtest-filter="$(filter)" $(opt)
